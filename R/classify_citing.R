...
#' @param save_dir The directory to save the script that records the user's classifications.  This script will have the name "classify_[object specified as df].R".
#'  
#' @details
...
#'  0  false positive
#'  1  agreeing with cited work or citing work as authority
...
examine_citing <- function(df, file) {
    for (i in seq_along(df$archive_link)) {
        ...
        cat(paste0("df$classification[df$id == '", 
                   df$id[i],
                   "'] <- ",
                   code),
            file = file, sep="\n", append = TRUE)
